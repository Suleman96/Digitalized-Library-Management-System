https://claude.ai/code/artifact/d572ad3d-6e70-42ff-84cc-df5ffef088c1?via=auto_preview# =============================================================================
...
# ---------------------------------------------------------------------------
def _update_model_choices(provider: str) -> tuple:
    """Update model dropdown and show/hide Ollama host input when provider changes."""
    models = PROVIDER_MODELS.get(provider.lower(), [])
    return (
        gr.update(choices=models, value=models[0] if models else None),
        gr.update(visible=(provider.lower() == "ollama")),
    )


def _ai_connect(provider: str, model: str, ollama_host: str) -> str:
    """Connect to the AI provider and build the agent. Returns status HTML."""
    if provider.lower() == "ollama" and ollama_host.strip():
        llm.set_ollama_host(ollama_host.strip())
    ...
    if llm.is_enabled:
        ...
        if not ok:
            suffix = (
                ...
                "Agent graph build failed — LangChain packages may be missing.</small>"
            )
        return f'<span class="ai-status-ok">{status}</span>{suffix}'
...
def _concierge_ai_status() -> str:
    if llm.is_enabled:
        provider = llm._provider.capitalize()
        model    = llm._model
        return (
            f'<div class="msg-ok" style="margin:0 0 .75rem;">'
            f'<span class="msg-tag">Connected</span>'
            f'{html_mod.escape(provider)} · {html_mod.escape(model)} — concierge is active.'
            f'</div>'
        )
    return (
        '<div class="msg-err" style="margin:0 0 .75rem;">'
        '<span class="msg-tag">Offline</span>'
        'AI not connected. Open <strong>AI Configuration</strong> above and connect '
        'Claude, OpenAI, Gemini, Groq, or local Ollama to activate the concierge.'
        '</div>'
    )
...
def _initial_ai_status() -> str:
    ...
    return (
        ...
        'AI offline — open AI Configuration to connect Claude, OpenAI, Gemini, Groq, or Ollama.</span>'
        '</div>'
    )
  ...
  <div class="hero-grid">
    <div class="hero-copy">
      ...
      <div class="hero-chip-row">
        ...
        <span class="hero-chip">AI librarian (Claude / OpenAI / Gemini / Groq / Ollama)</span>
...
""")
    ...
    with gr.Accordion("AI Configuration", open=False,
        ...
        gr.HTML("""<div style="padding:.4rem 0 .6rem;font-size:.88rem;color:var(--muted);line-height:1.6;">
          Connect an AI provider to unlock query expansion, match explanations,
          the conversational librarian, and advanced RAG stages (HyDE, multi-query, re-rank).
          <br><strong style="color:var(--text)">Groq</strong> is free and fast — great for demos.
          <strong style="color:var(--text)">Ollama</strong> is fully local — no API key needed.
        </div>""")
        with gr.Row():
            _init_provider = llm._provider if llm.is_enabled else "claude"
            ai_provider_radio = gr.Radio(
                choices=["claude", "openai", "gemini", "groq", "ollama"],
                value=_init_provider,
                label="Provider",
                scale=3,
            )
            _init_models = PROVIDER_MODELS.get(_init_provider, PROVIDER_MODELS["claude"])
        ...
        ollama_host_box = gr.Textbox(
            ...
            value=llm._ollama_host,
            placeholder="http://localhost:11434",
            info="Only shown for Ollama. Change if your server runs on a different host or port.",
            visible=(_init_provider == "ollama"),
        )

        ai_status_html = gr.HTML(
            f'<span class="ai-status-ok">{html_mod.escape(llm.status)}</span>'
            if llm.is_enabled else
            '<span class="ai-status-off">Not connected — AI features are disabled.</span>'
        )

        ai_provider_radio.change(
            fn=_update_model_choices,
            inputs=ai_provider_radio,
            outputs=[ai_model_dd, ollama_host_box],
        )
        ai_connect_evt = ai_connect_btn.click(
            fn=_ai_connect,
            inputs=[ai_provider_radio, ai_model_dd, ollama_host_box],
            ...
            with gr.Column(elem_classes="tab-wrap"):
                ...
                with gr.Row():
                    with gr.Column(scale=4):
                        ...
                        with gr.Group(elem_classes="filter-card"):
                            ...
                            with gr.Row():
                                ...
                                ai_search_chk = gr.Checkbox(
                                    ...
                                    info="Uses AI query expansion when a provider is connected. Auto-enables on connect.",
                                    scale=1,
                                )
                                # Wire AI connect → auto-check this box
                                ai_connect_evt.then(
                                    fn=lambda: gr.update(value=llm.is_enabled),
                                    outputs=ai_search_chk,
                                )
                            with gr.Row():
                        ...
                        with gr.Accordion("Advanced RAG Enhancements (requires AI)", open=False):
                            gr.HTML("""<p style='font-size:.85rem;color:var(--muted);margin:.2rem 0 .8rem;line-height:1.6;'>
                              All three options require a connected AI provider.
                              They add retrieval depth but increase response time.
                              <strong>HyDE</strong> and <strong>Multi-Query</strong> are pre-retrieval;
                              <strong>Re-rank</strong> is a local cross-encoder (no AI needed once downloaded).
                            </p>""")
                            with gr.Row():
                                use_hyde_chk = gr.Checkbox(
                                    label="HyDE — Hypothetical Document",
                                    value=False,
                                    info="AI writes an ideal book description first, then searches by that description. Better semantic match for abstract queries.",
                                )
                                use_mq_chk = gr.Checkbox(
                                    label="Multi-Query + RRF",
                                    value=False,
                                    info="AI generates 3 query variants; results are merged using Reciprocal Rank Fusion. Reduces blind spots from a single phrasing.",
                                )
                                use_rr_chk = gr.Checkbox(
                                    label="Cross-Encoder Re-rank",
                                    value=False,
                                    info="Re-scores the shortlist with a local cross-encoder model (ms-marco). Higher precision, ~0.5–2 s extra latency.",
                                )
